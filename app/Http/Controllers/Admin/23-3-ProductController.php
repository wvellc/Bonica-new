...
class ProductController extends Controller
{
    ...
    public function create()
    {
        ...
        $data['packet'] = Packet::select(
            DB::raw("CONCAT(id,'-',price) AS id"),'name')->Active()->pluck('name', 'id')->toArray();
    ...
    public function edit($id)
    {
        ...
        /*Packet*/
        //$data['packet'] = Packet::Active()->pluck('name', 'id')->toArray();
        $data['packet'] = Packet::select(
            DB::raw("CONCAT(id,'-',price) AS id"),'name')->Active()->pluck('name', 'id')->toArray();
        /*End Packet*/
    ...
    private function handleCreateOrUpdate($request, $edit = false, $id = null)
    {
        ...
        try {
            ...
            if($request->is_solitaire){
                $product->is_solitaire  = $request->is_solitaire;
            }
            ...
            ProductShapePacket::where('product_id', $product->id)->delete();
            if (!empty($request->shape_ids)) {
                foreach ($request->shape_ids as $value) {

                    $shape_data = array('product_id' => $product->id, 'shape_id' =>  $value);
                    $productShape = ProductShape::create($shape_data);

                    if(!empty($request->shape)){
                        foreach ($request->shape[$value] as $shape_packet) {
                            $shape_packet_arr = explode('-',$shape_packet[0]);
                            $shapepacket_data = array('product_id' => $product->id, 'shape_id' =>  $value, 'packet_id' =>  $shape_packet_arr[0],'weight' =>  $shape_packet[1],'pcs' =>  $shape_packet[2],'price' =>  $shape_packet[3]);
                            ProductShapePacket::create($shapepacket_data);
                        }
                    }
    ...
    public function deleteProductPacket(Request $request)
    {
        $id =  $request->packet_id;
        ProductShapePacket::where('id', $id)->delete();
