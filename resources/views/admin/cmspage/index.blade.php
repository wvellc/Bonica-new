			...
			<div class="col-12">
				<div class="card">
					<div class="card-header">
						...
						{{-- <small class="float-sm-right"><a class="btn btn-info bg-gradient-info" href="{{ route("admin.cmspage.create") }}">
							Add {{$module}}
						</a></small> --}}
					</div>
					<!-- /.card-header -->
...
<script>
		...
		var myTable =  $('#tbl_datatable').DataTable({
			...
			serverSide: true,
			searching: true,
			ajax: "{!! route('admin.cmspage.index') !!}",
			deferRender: true,
			lengthMenu: [
        ...
        function statusChange(id) {
            $.ajax({
                url: '{!! route("admin.cmspage.statusupdate") !!}',
                type: 'POST',
                data: {
                    "_token": "{{ csrf_token() }}",
                    "id": id
