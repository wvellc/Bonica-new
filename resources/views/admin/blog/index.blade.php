			...
			<div class="col-12">
				<div class="card">
                    <div class="card-header">
						...
						<small class="float-sm-right"><a class="btn btn-info bg-gradient-info" href="{{ route("admin.blog.create") }}">
							Add {{$module}}
						</a></small>
					</div>
					<!-- /.card-header -->
...
<div class="modal fade" id="modal-contact-view">
    <div class="modal-dialog modal-xl">
        ...
        <div class="modal-header">
            <h4 class="modal-title" id="modal-title">Blog Content</h4>
...
<script>
		...
		var myTable =  $('#tbl_datatable').DataTable({
           ...
           /*  ajax: {
                ...
                "data": function ( data )
                {
                    data.search_practitioner  = $("#practitioner").val();
                }
            }, */
			deferRender: true,
			lengthMenu: [
        ...
        /* $('#practitioner').on('change', function() {
            myTable.ajax.reload();
        });
        ...
        function statusChange(id) {
            $.ajax({
                url: '{!! route("admin.blog.statusupdate") !!}',
                type: 'POST',
                data: {
                    "_token": "{{ csrf_token() }}",
                    "id": id
        ...
        function showContent(id){
            ...
            success: function (data) {
                if(data.content != ""){
                    $('#content_view').html(data.content);
                }
                else{
                    $('#content_view').html('No Content Available');
                }
            }
            });
