					...
					<div class="row mt-4">
						...
						<div class="col-md-12 col-xl-5">
							<div class="detials-wrapper-box">
								...
								<div class=" mt-3">
									<h5>Diamond Shape</h5>
									<div class="shape-radio-design">
                                        @foreach ($product->ProductShapes as $productshape)
                                            <input onclick="getProductPriceImage();" type="radio" id="shape_{{$productshape->shape->id}}" name="shape" @if ($product->firstProductShape->shape_id == $productshape->shape->id) checked @endif value="{{$productshape->shape->id}}"/>
										    <label for="shape_{{$productshape->shape->id}}"><img src="{{ asset('uploads/shape/'. $productshape->shape->image) }}" alt="Round">{{$productshape->shape->name}}</label>
                                        @endforeach
									</div>
								</div>
                                @endif
                                @if ($product->color || $product->clarity)
								...
								<div class="">
									...
									<div class="square-radio-design">
                                        ...
                                        <input type="radio" id="size_{{$size[0]}}" name="ringSize" value="{{$size[0]}}" @if ($loop->first) checked @endif>
										<label for="size_{{$size[0]}}">{{$size[1]}}</label>
                                        @endif
                                        @endforeach
									</div>
								</div>
						...
						<div class="col-md-12 col-lg-6 text-center">
							<div class="hand-wrapper">
								...
								<div class="box-diamond-on-hand">
									<img id="diamondOnhand" src="{{ asset('images/shapes/ascher.png') }}" alt="image" class="diamond-on-hand" style="transform: scale(2.20);">
								</div>
					...
					<div class="row align-items-center justify-content-between">
						<div class="col-md-12 text-center ">
							<div class="section-title">
								<h2>Bonica Assurance to you</span></h2>
							</div>
    ...
    $.ajax({
            ...
            success: function (data) {
                if(data.msg == 'success'){
                    ...
                    $('#diamondOnhand').attr('src', "{{asset('images/shapes')}}/"+$("label[for='shape_"+shape_id+"']").text().toLowerCase()+'.png');
